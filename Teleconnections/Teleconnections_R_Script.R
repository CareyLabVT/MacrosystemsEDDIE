...
install.packages('sp') 
# NOTE: You may get output that says, "There is a binary version available. Do 
  #  you want to install from sources that need compilation? y/n" If this pops 
  #  up, type 'y' (without the quotes) and hit enter. You may now be prompted to 
  #  download the command line developer tools in a pop-up window. Command line 
  #  developer tools is a program used to run modeling software. Click Install 
...
# Download the GLMr software. This may take a few minutes. If downloaded 
  #  successfully, you should see "DONE (GLMr)" at the end of the output.
...
library(glmtools) 
# Load the two packages that you need to analyze GLM output. NOTE: you may get 
  #  lots of red output messages- if the packages loaded successfully, the text 
  #  will start with: "This information is preliminary or provisional..." 
...
LakeName <- 'Name' ##!! Change 'Name' to match the lake you and your partner 
  #  selected. The lake name options are: 'Barco', 'Crampton', 'Falling Creek', 
  #  'Mendota', 'Prairie Pothole', 'Suggs', 'Sunapee', and 'Toolik'
...
# To find your folder path, navigate to the 'teleconnections' folder on your 
  #  Desktop, then open the Lakes folder. Right click on the folder that matches 
  #  your model lake, then select Properties (Windows) or Get Info (Mac). Look 
  #  under Location (Windows) or Where (Mac) to find your path (examples below):
  #  Windows: C:/Users/KJF/Desktop/teleconnections/Lakes/LakeName
  #  Mac: Users -> careylab -> Desktop -> teleconnections -> Lakes -> LakeName

sim_folder <- '/Users/cayelan/Desktop/teleconnections/Lakes/LakeName' 
##!! Edit this line to define the sim_folder location for your model lake. You 
  #  will need to change the part after Users/ to give the name of your computer 
  #  (e.g., my computer name is cayelan, but yours will be different!) AND change 
  #  the LakeName part to be the name of your model lake.

setwd(sim_folder) 
# This line of code resets your working directory to the sim_folder. The point of 
  #  this step is to make sure that any new files you create (e.g., plots of 
  #  model output) end up together in this folder.

nml_file <- paste0(sim_folder,"/glm2.nml") 
# This step sets the nml_file for your simulation to be in the sim_folder

nml <- read_nml(nml_file) # Run this line to read in your nml file 

print(nml) 
# This shows you what is in your nml file.  This is the 'master script' of the 
  #  GLM simulation; the nml file tells the GLM model all of the initial 
...
# This command plots the meterological input data for the simulation: short wave 
  #  & long wave radiation, air temp, relative humidity, etc. for the duration of 
  #  the simulation. 
...
# (1) Extract ice cover data:
ice <- get_var(baseline, "hice") 
# (2) Rename column so we know it's from the Baseline scenario
...
# This plots your simulated lake temperatures in a heat map, where time is 
  #  displayed on the x-axis, lake depth is displayed on the y-axis, and the 
  #  different colors represent different water temperatures. 
...
# Note that depending on its location, your lake may not experience any days 
  #  with ice! 

########## ACTIVITY B - OBJECTIVE 3 ############################################
# For Activity B, you will work with your partner to model your lake under two 
  #  teleconnections scenarios of simulated El Nino conditions. To do this, we 
  #  will use observed, historical climate data to estimate how much warmer or 
  #  cooler your lake's local air temperature would likely be during an El Nino 
  #  year. 

# Install this package to read Excel (.xlsx) files directly into R
install.packages('readxl') 

# Install these packages to make manipulating data easy
install.packages('dplyr') 
install.packages('tidyr')

# Now load the packages you just installed
library(readxl) 
library(dplyr)
library(tidyr)
...
# The data we'll be using to estimate El Nino temperature offsets is 
  #  "Air Temp Mean (°C)", or the annual average air temperature for your lake.

# Let's look at how annual air temperature differs between El Nino years and 
  #  non-El Nino years for your lake. First, we'll subset the data into El Nino 
  #  years and neutral (neither El Nino nor La Nina) years.
# First, we define Neutral years
Neutral_years <- subset(annual_temp, Type == "Neutral") 
# Then we define El Nino years
ElNino_years <- subset(annual_temp, Type == "ElNino") 

# Visualize your lake's patterns in air temperature over time using the commands 
  #  below:
...
legend("topleft",c("All Years", "El Nino", "Neutral"), pch=16, 
       col=c("gray70", "red", "black")) 
# Now, we need to estimate how much warmer or colder a typical El Nino year is 
  #  compared to a typical neutral year. 

# We'll do that by using linear regressions between temperature and year that 
  #  are fit separately for neutral years and El Nino years
...
# Save the model slope
slope_Neutral = summary(mod_Neutral)$coeff[2] 

# Save the model intercept
int_Neutral = summary(mod_Neutral)$coeff[1] 

# To estimate the air temperature in 2013 (the year of our model, which was a 
  #  neutral year), we use the equation for a straight line (y = m*x + b), where 
  #  x = 2013, m is the slope_Neutral, and b is the int_Neutral. We're solving 
  #  for y!
# Notice this form is y = (m*x) + b
Neutral_2013 = (slope_Neutral * 2013) + int_Neutral 

# Run this line to have R print out the value you just calculated
print(Neutral_2013) 
...
# Now, we plug in the slope and intercept from the El Nino regression line to 
  #  estimate what the air temperature would likely have been if 2013 (the year 
  #  of our model) was a typical El Nino year ("typical" meaning  that it fell 
  #  on the El Nino regression line we just fit)
ElNino_2013 <- (slope_ElNino * 2013) + int_ElNino

# Run this line to have R print out the value you just calculated
print(ElNino_2013) 

# Now, use the commands below to add the Neutral and El Nino regression lines to 
  # your plot: 
...
# Run this line to estimate the typical El Nino offset
typicalOffset_degrees <- ElNino_2013 - Neutral_2013 
# Run this line to have R print out your temperature offset
print(typicalOffset_degrees) 

# Now, we need to create a new meteorological driver file for GLM that has air
  #  temperatures adjusted to reflect our lake's estimated offset for a typical 
  #  El Nino year

# We can do that all in R (no Excel needed, whoo!!). First, we read in the 
  #  original meteorological driver data for GLM using the following commands: 
...
##!! The printout here should list your NEW meteorological file name for your 
  #  El Nino scenario. If it doesn't, make sure you pressed the Save icon (the 
  #  floppy disk) after you changed your glm2.nml file.
...
# Run your GLM model for your "typical" El Nino scenario 
run_glm(sim_folder, verbose=TRUE) 

# Again, we need to tell R where the output.nc file is so that we can plot and 
  #  analyze the model output. We tell R where to find the output file using the 
  #  command below:
Typical_ElNino <- file.path(sim_folder, 'output.nc') 
# This defines the output.nc file as being within the sim_folder. Note that 
  #  we've called this output "Typical_ElNino" since it is the output from our 
  #  "typical" El Nino teleconnections simulation.

# As before, we want to save the model output of the daily water temperatures 
  #  from the surface and hypolimnion, and the daily ice cover during our 
  #  typical El Nino simulation, to compare to our baseline scenario. 

#  Use this command to extract the surface and bottom water temperatures:
scenario2_temp <- get_temp(file= Typical_ElNino, reference= 'surface', 
                           z_out= c(0, min(lakeDepth$surface_height)))
...
##!! To check that your El Nino scenario ran correctly, run the command below, 
  #  and compare the water temperatures between your baseline and typical El 
  #  Nino scenario. They'll likely be similar, but if they're exactly the same, 
  #  something might have gone wrong in setting up your El Nino scenario (likely 
  #  with changing the glm2.nml file!)
View(lakeTemp_output)

# Extract ice thickness data from the typical El Nino scenario and rename the 
  #  ice column: 
scenario2_ice <- get_var(baseline, "hice") 
ice["Typical"] <- scenario2_ice[2]  

# Now we'll plot the water temperature heatmap for the El Nino scenario using 
  #  the commands you learned above: 

# Create a heatmap of water temperature. How does this compare to your baseline?
plot_temp(file=Typical_ElNino, fig_path=FALSE) 

# To plot surface ice during your model year, run this command:
plot(x = ice$DateTime, y = ice$Typical, type = 'l',  xlab = "Date", 
     ylab = "Ice thickness (m)")

# Note that it might be difficult to see subtle changes between scenarios using 
  #  these figures. In Activity C, we will make different plots that make it 
  #  easier to see differences between scenarios.
...
# Take a look at the data sheet by running this line
View(offsets) 
...
# Here, we can use R to tell us which year had the largest offset (i.e., which 
  #  year was the strongest El Nino year)
...
# Now that we know the offset for our "strong" El Nino scenario, we need to 
  #  modify the meteorological driver file once more. We'll repeat the same 
  #  steps we used when creating our "typical" El Nino scenario.
...
##!! Once again, you need to edit the glm2.nml file to change the name of the 
  #  inputmeteorological file so that it reads in the new, edited file for your 
  #  "strong" El Nino scenario, not the default "met_hourly.csv" or the file 
  #  from your "typical" El Nino scenario "met_hourly_scenario2.csv".  

##!! Open the nml file by clicking 'glm2.nml' in the Files tab of RStudio, then 
  #  scroll down to the meteorology section, and change the 'meteo_fl' entry to 
  #  the new met file name ('met_hourly_scenario3.csv'). Save your modified 
  #  glm2.nml file.
...
nml <- read_nml(nml_file)  
get_nml_value(nml, 'meteo_fl') 
# The printout here should list your NEW meteorological file for your "strong" 
  # El Nino scenario. If it doesn't, make sure you pressed the Save icon (the 
  #  floppy disk) after you changed your glm2.nml file.

# You can now run the model for your "strong" El Nino scenario using the updated 
  #  nml file using the commands below. Exciting!

# Run your GLM model for your "strong" El Nino scenario 
run_glm(sim_folder, verbose=TRUE) 

# Once more, we need to tell R where the output.nc file is, using the command 
  #  below:
Strong_ElNino <- file.path(sim_folder, 'output.nc') 

# As before, we want to save the model output of the daily surface  and 
  #  hypolimnion lake temperature, and daily ice cover during our "strong" El 
  #  Nino  scenario, so we can compare to our baseline scenario and "typical" El 
  #  Nino scenario. 

#  Extract surface water temperature:
scenario3_temp <- get_temp(file= Strong_ElNino, reference= 'surface', 
                           z_out= c(0, min(lakeDepth$surface_height)))
...
plot(x = ice$DateTime, y = ice$Strong, type = 'l',  xlab = "Date", 
     ylab = "Ice thickness (m)")

# Note that as before, it might be difficult to see subtle changes between 
  #  scenarios using these figures. In Activity C below, we will make different 
  #  plots that make it easier to visualize these differences
...
# Let's first compare the water temperatures across the three scenarios with a 
  #  line plot. The command below plots DateTime vs. Observed data from the 
  #  baseline model in black: 
attach(lakeTemp_output)
plot(x = DateTime, y = Baseline_Surface_Temp, type="l", col="black", 
     xlab="Date", ylab="Surface water temperature (C)", lwd=2, ylim=c(0,40))  
# !! Note that the command ylim=c(0, 40) tells R what you want the minimum and 
  #  maximum values on the y-axis to be (here, we're plotting from 0-40 degrees 
  #  C). Adjust this range and rerun the plotting commands to make sure your 
  #  data are clearly visualized in the plot.

# This command adds an orange line of the surface temperature output from the 
  #  typical El Nino scenario
lines(x = DateTime, y = Typical_ElNino_Surface_Temp, lwd=2, col="orange2") 

# This command adds a red line of the surface temperature output from the strong 
  #  El Nino scenario
...
legend("topleft",c("Baseline", "Typical El Nino", "Strong El Nino"), 
       lty=c(1,1,1), lwd=c(2,2,2), col=c("black","orange2", "red2")) 
legend("topright", c("Surface", "Bottom"), lty=c(1,2), lwd=c(2,2))

# We can also look at how the duration of ice cover changed between the three 
  #  scenarios. To do this, we'll use the following string of commands to 
  #  calculate the number of days with ice in each scenario, based on the ice
  # data we saved already
...
# Now we can plot the number of days with ice cover as a function of our lakes' 
  # temperature offset:
attach(iceDuration)
plot(x = Offset, y = iceDays, type="b", pch=16, cex=2, 
     col=c("black", "orange1", "red2"), xlab= "Temperature offset (C)", 
     ylab= "Days with surface ice", ylim=c(0,365))
